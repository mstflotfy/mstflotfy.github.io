...
description: I am on a learning journey to build my own apps. I started it by exploring Figma, now I think I need to move onto coding, and I think that learning the command line is a great starting point. This is my cmd line study notes. I tried to simplify getting started with the cmd line.
---
...
We use a GUI (Graphical UI) to interact with our computer.
...
But why would you want to do that? Why should **even** you bother?
...
It's a skill that you'll need to learn anyways if you want to build your own apps.
...
A fourth example. I came across another tool called [revealjs](https://revealjs.com){:target="_blank" rel="noreferrer"}. It allows you to use HTML or Markdown to make presentations that look great, just by typing some text! And I also had to use the cmd line to install it and to use it.
...
Through package managers you can download and install tools that you can use daily, that are not just for developers.
...
`pwd` returns a path for you. Then the prompt and cursor again to show you that you can type another command if you want.
...
Therefore a full path to any file or folder starts from the **root**. This called an **absolute path**.

The **home directory** is the a directory that holds your personal files and folders. It contains folders like **Desktop**, **Downloads**, **Documents**, among other things. 
...
`ls -a` The `-a` option will include dot files, which are hidden files, in the list.
...
- `-G` gives colored output and folders have a different color than files. 
-  `-a` shows hidden files
- `-l` gives a long list with more info about the file, that will come in handy in the future as you expand your knowledge. Including file type, permissions, and file size.
-  `h` makes the size of files human readable by adding K for Kilobytes, or G for Gigabytes and so on. 
...
- `<arguments>` Inputs some commands need to run. like a path to file or directory. If the cmd is to list then I can give directories as input to be listed.
...
If you are at the the home directory `~`. The Documents directory is already inside it, no need to give an absolute path, we can give a relative path instead. I can just do it this way directly `ls Desktop` This is called a relative path. 

If an absolute path uses the root as the reference a relative path uses our current working directory as the reference. 

Current working directory is referred to with a `.`, so you can `ls .` but there is no need to write it here as the `ls` command already lists the current working directory if given no argument.
...
`cd` Stands for change working directory. Use it to navigate through your folders, to head to any folder on your computer. 

You can use a realtive path; `cd Downloads`. 
...
`open ..` Open the partent directory using GUI.

`open ~/Desktop` Open any folder on your computer by giving an absolute path to its destiination.

`open Documents` Use a relative path to open a folder. This is just like `open ./Documents`, using your current wroking directory `.` as the starting point instead of the root `/` directory.

If you have a `screenshots` directory at your Desktop, you can open it inside GUI, and look the images by typing this: `$ open ~/Desktop/screenshots`
...
Now that you know where you are, and you can list files and directories, and you can navigate between directories, let's see how we can organize our directories and files.
...
`mv image.png photo.png` We can also use the mv command to rename files or folder. This command will rename the file *image.png* to *photo.png*. 
...
Also, you can rename while copying, and copy in the same directory to make duplicates. The `-r` ooption will copy folders with all files and directories inside them.  
...
The only thing this command does is remove empty folders. Which could be useful if you have a bunch of empty folders you want to delete. You can run this command on all files and folders in a directory, knowing that only empty directories will be deleted. 
...
Be careful with this, as the files are not moved to the **trash** but removed permenantly without asking for confirmation. And you can't get them back!

`rm ~/Documents/myImage.jpeg` This will remove the file **myImage.jpeg** from the **Documents** directory.

`rm -r ~/Desktop/myFolder` This will permenantly delete the directory **myFolder** from the **Desktop** directory. The `r` option, stands for recursive, it will make the remove command delete a folder with all files and foders inside it. Be careful son!

To avoid accidents using the `rm` command, use the `mv` command instead to move the files and directories you want to delete into the system's trash folder, for mac it's `~/.Trash`. This would allow you to restore your files or remove themm safely.

### `Wildcards`

Wildcards will make your command apply to every file that matches a specific pattern.

There are different ways to make patters, but usually, you can target files that end in a specific extension, like this `mv *.png` This command will permenantly delte every single file that has the **.png** extension, that ends in **.png**, inside the current working directory. 

Or, you can target files that start with a specific word, like this `cp mstf* ~/Documents/myFiles`. This command will copy every file that starts with **mstf**, and send the copies to **myFiles** folder, at the **Documents** directory.
...
```bash
 ...
 ```
Creating a new text file using GUI can take some steps, using the command line you can create a text file, or bunch of them, on the fly.
...
`open file.txt` This will open **file.txt**, using the default app and you can edit it and save, then get back to the terminal.
...
Use the `cat` command to have a quick peek insde text files.

It stands for concatenate, to link or connect together. But, it's simplest use case is a quick peek into a text file.
...
Echo is a simple command that returns whatever argument you give it. It takes your input and turns it into output. While this might at first seem useless. It can be used to quickly pass text into a file, without first opening it! 
...
That's why most people's first encounter with Vim, myslef included, is getting stuck and not being able to exit!!

Vim needs it's own intro so, for now, just use `open`, or `nano` and in the future we can dive into Vim.
...
An alias is just shortcut to long commands that you repeatedly use.

Here's an example of an alias I use; I write `lsg` to execute the command `ls -GhalS`. I set it this way; `alias lsg='ls -GhalS'`

You can also use aliases to `cd`, change directory (head to), your most used folders. For example: `alias myFolder='cd ~/Documents/myFolder`.

Typing an alias directly into the terminal, will set it only for this session. Once the this terminal window is closed, the alias will be gone! 

To make an alias permenant you need to add it into a file that manages the settings of your terminal. This file name depends on which shell you use, most probably your system will be using the **bash** shell, or if you have a new mac it will be **zsh**.
...
If you want a list of all the aliases you have currently set. You can just write the command `alias`. This would be useful if youforgot what aliases you've set.
...
If you want a minimal and clean prompt add this `ps1='$ '` , in an empty line, to your `~/.bashrc`  Or `~/.zshrc` file
...
### 1. Command Line Cheat sheets

There is no need to memorize any command.

Understand what a command does, when you get introduced to it, and how it works, then choose a cheatsheet to get back to when you forget it. Or even better, make your own cheatsheet, with your favorite commands.
...
A very recommended book that dives deep into the the command line and that is suited for beginners.

### 4. Online Class

#### The Missing Semester of Your CS Education, a class by MIT.

This class is a part of the the [Open source society Uni](https://github.com/ossu/computer-science){:target="_blank" rel="noreferrer"}'s path to a free self taught education in computer science. 
